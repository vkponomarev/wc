    ...
    <form action="./<?php if (!Yii::$app->params['embed']): ?>#result<?php endif; ?>">
        ...
        <div class="form-content">
            ...
            <div class="col-xs-12 col-sm-6 align-mid">
                ...
                <select id="cycle-length-from" name="mother-hand" class="form-control select-extended">

                    <?php for ($i = 1; $i <= 2; $i++): ?>
                        <option value="<?= $i ?>"
                            <?php if ($getParams['motherHand']) {
                                if ($i == $getParams['motherHand'])
                        ...
                        >
                            <?php if ($i == 1) :?>
                            <?= Yii::t('app','Right-handed')?>
                            <?php endif; ?>

                            <?php if ($i == 2) :?>
                                <?= Yii::t('app','Left-handed')?>
                            <?php endif; ?>
                ...
                <select id="cycle-length-from" name="father-hand" class="form-control select-extended">

                    <?php for ($i = 1; $i <= 2; $i++): ?>
                        <option value="<?= $i ?>"
                            <?php if ($getParams['fatherHand']) {
                                if ($i == $getParams['fatherHand'])
                                    echo 'selected="selected"';
                            } elseif ($i == 1) {
                                echo 'selected="selected"';
                            }
...
<div class="<?php if (($result['viewResult'] <> 0) or Yii::$app->params['embed']): ?>result-div-on<?php else: ?>result-div-off<?php endif ?>">
    ...
    <div class="result-div-text">
            <span class="result-pre-text">
                <?= Yii::t('app', 'Probability table of a right-handed or left-handed child:') ?> <br>
                <?= Yii::t('app', 'Right-handed:') ?> - <?= $result['childHand'][1] ?> %<br>
                <?= Yii::t('app', 'Left-handed:') ?> - <?= $result['childHand'][2] ?> %<br>
