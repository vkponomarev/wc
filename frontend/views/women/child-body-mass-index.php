        ...
        <div class="form-content">
            ...
            <div class="col-xs-12 col-sm-6 align-mid">
                ...
                <select id="cycle-length-from" name="child-age-years" class="form-control select-extended">
                    ...
                    <?php for ($i = 0; $i <= 11; $i++): ?>
                        <option value="<?= $i ?>"
                            <?php if ($getParams['childAgeYears']) {
                ...
                <select id="cycle-length-from" name="child-age-months" class="form-control select-extended">
                    ...
                    <?php for ($i = 0; $i <= 11; $i++): ?>
                        <option value="<?= $i ?>"
                            <?php if ($getParams['childAgeMonths']) {
                ...
                <input id="cycle-length-from" name="height" class="form-control select-extended" type="text"
                       size="40"
                       value="<?php if ($getParams['height']): ?><?= $getParams['height'] ?><?php else: ?>70<?php endif ?>">
...
<div class="<?php if (($result['viewResult'] <> 0) or Yii::$app->params['embed']): ?>result-div-on<?php else: ?>result-div-off<?php endif ?>">
    ...
    <div class="result-div-text">
            <span class="result-pre-text">
                <?php if ($result['BMI'] <> 0): ?>
                    <?= Yii::t('app', 'Body mass index of a child:') ?>
                    <?= $result['BMI'] ?> <?= Yii::t('app', 'kilogram / meter') ?><sup><small>2</small></sup><br><br>
                <?php endif; ?>
