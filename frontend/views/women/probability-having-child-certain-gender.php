    ...
    <form action="./<?php if (!Yii::$app->params['embed']): ?>#result<?php endif; ?>">
        ...
        <div class="form-content">
            ...
            <div class="col-xs-12 col-sm-6 align-mid">
                ...
                <select id="cycle-length-from" name="gender" class="form-control select-extended">

                    <?php for ($i = 1; $i <= 2; $i++): ?>
                        <option value="<?= $i ?>"
                            <?php if ($getParams['gender']) {
                                if ($i == $getParams['gender'])
...
<div class="<?php if (($result['viewResult'] <> 0) or Yii::$app->params['embed']): ?>result-div-on<?php else: ?>result-div-off<?php endif ?>">
    ...
    <div class="result-div-text">
            <span class="result-pre-text">
                ...
                <?= Yii::t('app', 'The probability of having a baby of a certain gender:') ?><br>
                <?= Yii::t('app', 'Russia:') ?> <?= $result['childCertainGender'][1]?> %<br>
                <?= Yii::t('app', 'Ukraine:') ?> <?= $result['childCertainGender'][2]?> %<br>
                <?= Yii::t('app', 'Kazakhstan:') ?> <?= $result['childCertainGender'][4]?> %<br>
                <?= Yii::t('app', 'USA:') ?> <?= $result['childCertainGender'][3]?> %<br>
                <?= Yii::t('app', 'Iceland:') ?> <?= $result['childCertainGender'][5]?> %<br>
