...
class LanguageSelection
{
    ...
    function create($path, $partialsConfig)
    {
        ...
        $template = (new LanguageSelectionTemplateMain())->template($partialsConfig);

        //делается попытка создать его

        $fp = fopen($path . '/language-selection.php', "w");
        ...
        $template2 = (new LanguageSelectionTemplateDropDown())->template($partialsConfig);

        //делается попытка создать его

        $fp = fopen($path . '/_drop-down.php', "w");
        // записываем в файл текст
        fwrite($fp, $template2);
        // закрываем
        fclose($fp);
        ...
        $template3 = (new LanguageSelectionTemplateName())->template($partialsConfig, $path);
